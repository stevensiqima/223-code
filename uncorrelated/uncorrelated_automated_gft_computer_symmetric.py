# Designed to compute GFT in cases where buyer valuation is inputted by user, ane seller valuation is set to be
# the symmetric version of the buyer valuation. Buyer/seller densities are also symmetric, with buyer/seller densities
# iterated through all possible densities in a given range (inputted by user). Able to run faster than uncorrelated_automated_gft_computer.py
...
for i in range(0, x**n):
    ...
    # Computes GFT and adds to datalist
    data = gft_functions.gftcomputersymmetric(sellervaluation, sellerprobability, buyervaluation, buyerprobability)
...
with open('/Users/stevenma/PycharmProjects/gainsfromtradeupdated/data/symmetric n='+str(n)+', x='+str(x)+'.csv', 'a') as csvfile:
    ...
    writer.writerow(["Buyer densities", "Ratio of BOM + SOM to FB"+"\n"])
