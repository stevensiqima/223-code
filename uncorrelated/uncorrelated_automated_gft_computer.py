# Designed to compute GFT in cases where buyer/seller valuations are fixed by user input and buyer/seller densities
...
buyervaluation = [1, 2, 3, 4, 5]
sellervaluation = [0, 1, 2, 3, 4]
...
for i in range (0, x**n):
    ...
    for k in range(0, y**m):
        ...
        row = [buyerprobability, sellerprobability, (data[0]+data[1])/data[2]]
        datalist.append(row)
...
with open('/Users/stevenma/PycharmProjects/gainsfromtradeupdated/data/buyer='+str(n)+', seller='+str(m)+', x='+str(x)+', y='+str(y)+'.csv', 'a') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(["Buyer support:",buyervaluation, "Seller support:", sellervaluation])
    writer.writerow(["Buyer densities", "Seller densities", "Ratio of BOM + SOM to FB"+"\n"])
